...
// and asserts the return value is equal to dynamic_cast< IHasAttributes * >( pEntity ).
inline IHasAttributes* GetAttribInterface(CBaseEntity* pEntity)
{
    IHasAttributes* pAttribInterface = pEntity ? pEntity->GetHasAttributesInterfacePtr() : nullptr;
...
public:
    ...
    template <class T> static T AttribHookValue(T TValue, const char* pszAttribHook, const CBaseEntity* pEntity, CUtlVector<CBaseEntity*>* pItemList = NULL, bool bIsGlobalConstString = false)
    {
        ...
        IHasAttributes* pAttribInterface = GetAttribInterface(const_cast<CBaseEntity*>(pEntity));
        AssertMsg(pAttribInterface, "Attribute hook set on an entity that does not support attributes.");
...
private:
    ...
    {
        Assert(pszAttribHook && pszAttribHook[0]);
        Assert(pEntity);
        Assert(pAttribInterface);
        Assert(GetAttribInterface(const_cast<CBaseEntity*>(pEntity)) == pAttribInterface);

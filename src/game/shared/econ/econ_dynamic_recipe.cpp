...
// This pattern was chosen so that it is not a valid user-input string,
// not a sensical float bit pattern, a common int bit pattern, or meaningful Unicode.
const char* g_pszAttrEncodeSeparator = "|\x01\x02\x01\x03|\x01\x02\x01\x03|";


//-----------------------------------------------------------------------------
// CRecipeComponentMatchingIterator constructor
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// OnIterateAttributeValue: Checks the dynamic recipe component attribute value.
//-----------------------------------------------------------------------------
bool CRecipeComponentMatchingIterator::OnIterateAttributeValue(const CEconItemAttributeDefinition* pAttrDef,
    const CAttribute_DynamicRecipeComponent& value)
{
    // Do not count if source and target are the same.
    if (m_pSourceItem && m_pTargetItem && m_pSourceItem->GetID() == m_pTargetItem->GetID())
        return true;

    // If pTargetItem is non-null and the attribute doesn't match, skip.
    if (m_pTargetItem != nullptr && !DefinedItemAttribMatch(value, m_pTargetItem))
        return true;

    // Skip if target item exists but is not usable in crafting.
    if (m_pTargetItem != nullptr && !m_pTargetItem->IsUsableInCrafting())
        return true;

    // Accumulate outputs versus inputs.
...
//-----------------------------------------------------------------------------
// DefinedItemAttribMatch: Returns true if the item matches the attribute criteria.
//-----------------------------------------------------------------------------
bool DefinedItemAttribMatch(const CAttribute_DynamicRecipeComponent& attribValue, const IEconItemInterface* pItem)
{
    ...
    // Cache item definition and quality for efficiency.
    const uint32 itemDef = pItem->GetItemDefIndex();
    const uint32 itemQuality = pItem->GetQuality();

    // If fulfilled count equals required, no further matches are needed.
    ...
    if ((attribValue.component_flags() & DYNAMIC_RECIPE_FLAG_PARAM_ITEM_DEF_SET) &&
        (attribValue.def_index() != itemDef))
        return false;

    if ((attribValue.component_flags() & DYNAMIC_RECIPE_FLAG_PARAM_QUALITY_SET) &&
        (attribValue.item_quality() != itemQuality))
        return false;

    // If ALL attributes must be present:
    ...
    // If ANY one attribute is required:
...
//-----------------------------------------------------------------------------
// DecodeAttributeStringIntoAttributes: Splits the encoded attribute string and
// converts each pair into an attribute_t entry.
//-----------------------------------------------------------------------------
...
{
    CUtlStringList vecAttributeStrings;  // Automatically freed.
    ...
    {
        AssertMsg(0, "%s: Uneven count of encoded attribute strings!", __FUNCTION__);
        return false;
    }
    ...
    for (int j = 0; j < vecAttributeStrings.Count(); j += 2)
    {
        ...
        if (!pAttrDef->GetAttributeType()->BConvertStringToEconAttributeValue(pAttrDef, pszAttribValue, &attrib.m_value))
            return false;
    }
...
//-----------------------------------------------------------------------------
// DecodeItemFromEncodedAttributeString: Applies the decoded attributes and other flags
// to the item as described by the dynamic recipe component attribute.
//-----------------------------------------------------------------------------
bool DecodeItemFromEncodedAttributeString(const CAttribute_DynamicRecipeComponent& attribValue, CEconItem* pItem)
{
    Assert(pItem);

    // Set item definition if flagged; otherwise use a generic name.
    if (attribValue.component_flags() & DYNAMIC_RECIPE_FLAG_PARAM_ITEM_DEF_SET)
    {
    ...
    // Set item quality based on flag.
    if (attribValue.component_flags() & DYNAMIC_RECIPE_FLAG_PARAM_QUALITY_SET)
    {
        pItem->SetQuality(attribValue.item_quality());
        if (!(attribValue.component_flags() & DYNAMIC_RECIPE_FLAG_PARAM_ITEM_DEF_SET) && pItem->GetQuality() == AE_UNIQUE)
        {
            CAttribute_String attrStr;
    ...
    // Decode and apply attributes.
    CUtlVector<CEconItem::attribute_t> vecAttribs;
    if (!DecodeAttributeStringIntoAttributes(attribValue, vecAttribs))
    {
        AssertMsg(0, "%s: Unable to decode dynamic recipe attributes on item %llu", __FUNCTION__, pItem->GetID());
        return false;
    }

    FOR_EACH_VEC(vecAttribs, j)
    {
        const CEconItemAttributeDefinition* pAttr = GetItemSchema()->GetAttributeDefinition(vecAttribs[j].m_unDefinitionIndex);
        Assert(pAttr);
        const ISchemaAttributeType* pAttrType = pAttr->GetAttributeType();
        pAttrType->LoadEconAttributeValue(pItem, pAttr, vecAttribs[j].m_value);
